...
void display(void) {
	...
	if (flag_draw_screen) {
		set_material_screen();
		glUniform1i(loc_texture, TEXTURE_ID_PICTURE);
	...
	ModelViewMatrix = glm::rotate(ViewMatrix, -rotation_angle_tiger, glm::vec3(0.0f, 1.0f, 0.0f));
	ModelViewMatrix = glm::translate(ModelViewMatrix, glm::vec3(250.0f, 0.0f, 0.0f));
	ModelViewMatrix = glm::scale(ModelViewMatrix, glm::vec3(100.0f, 100.0f, 100.0f));

	if (timestamp_scene % 360 < 90 && r_flag == 0)
		ModelViewMatrix = glm::rotate(ModelViewMatrix, 4 * (timestamp_scene % 360) * TO_RADIAN, glm::vec3(0.0f, 1.0f, 0.0f));
	ModelViewProjectionMatrix = ProjectionMatrix * ModelViewMatrix;
	ModelViewMatrixInvTrans = glm::inverseTranspose(glm::mat3(ModelViewMatrix));
	...
	if (r_flag) {
		...
		ModelViewMatrix = glm::rotate(ViewMatrix, -rotation_angle_tiger, glm::vec3(0.0f, 1.0f, 0.0f));
		ModelViewMatrix = glm::translate(ModelViewMatrix, glm::vec3(20.0f, 0.0f, -40.0f));
	...
	if (flag_texture_mapping == 1) glUniform1i(loc_texture, TEXTURE_ID_TIGER);
		...
		glUniform1i(loc_texture, TEXTURE_ID_RAINBOW);
	ModelViewMatrix = glm::translate(ViewMatrix, glm::vec3(-100.0f, 0.0f, 300.0f));
	ModelViewMatrix = glm::scale(ModelViewMatrix, glm::vec3(50.0f, 30.0f, 30.0f));
	...
	if (flag_texture_mapping == 1) glUniform1i(loc_texture, TEXTURE_ID_TIGER);
		...
		glUniform1i(loc_texture, TEXTURE_ID_RAINBOW);
	ModelViewMatrix = glm::translate(ViewMatrix, glm::vec3(-200.0f, 0.0f, -200.0f));
	ModelViewMatrix = glm::scale(ModelViewMatrix, glm::vec3(5.0f, 5.0f, 5.0f));
...
void keyboard(unsigned char key, int x, int y) {
	...
	case 'a':
		...
		if (flag_tiger_animation) {
			...
			fprintf(stdout, "Animate start\n");
		}
		else
			fprintf(stdout, "Animate end\n");
		break;
	case 't':
	...
	case 'd':
		...
		camera.pos[0] = 150.0 * PRP_distance_level; camera.pos[1] = 100.0 * PRP_distance_level;  camera.pos[2] = 250.0 * PRP_distance_level;
		set_ViewMatrix_from_camera_frame();
	...
	case 's':
		...
		glutPostRedisplay();
		break;
	case 'x':		// x 누르면 x축 중심으로 회전
		flag_translation_axis = X_AXIS;
		break;
	case 'y':		// y 누르면 y축 중심으로 회전
		flag_translation_axis = Y_AXIS;
		break;
	case 'z':		// z 누르면 z축 중심으로 회전
...
void main(int argc, char* argv[]) {
	...
	char messages[N_MESSAGE_LINES][256] = { "    - Keys used: 'e', 'e', 'm', 'a', 't', 'f', 'c', 'd', 'y', 'u', 'i', 'o', 'ESC'" };
