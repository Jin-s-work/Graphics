...
#define accelate 0.001
#define ROT_SPEED 0.2f
#define update_rate 10

GLfloat carspeed[3] = { 0.2f , -0.2f, 0.2f };
GLfloat carindex[3] = { -300.0f, 400.0f, -200.0f };
GLfloat carheight[3] = { 250.0f, 0.0f, -250.0f };

GLfloat car2speed[3] = { -0.2f , 1.0f, 0.2f };
GLfloat car2index[3] = { 300.0f, 400.0f, 200.0f };
GLfloat car2height[3] = { 150.0f, 70.0f, -150.0f };

GLfloat custspeed[3] = { 0.2f , 0.5f, -0.2f };
GLfloat custindex[3] = { 100.0f, -200.0f, 100.0f };
GLfloat custheight[3] = { 200.0f, -100.0f, -100.0f };
...
float hat_height = 0.0f;
float hat_speed = 0.5f;

float cocktail_x = 120.0f;
float cocktail_y = 0.0f;
float cocktail_x_speed = 0.3f;
float cocktail_y_speed = 0.3f;

float cake_x = -80.0f;
float cake_y = 20.0f;
float cake_x_speed = 0.3f;
float cake_y_speed = 0.3f;

float custom_scale = 0.0f;
float custom_rotation = 0.0f;
int custom_flag = 0;
...
#define CUSTOM_BODY 0
#define CUSTOM_RIGHTHAT 1
#define CUSTOM_LEFTHAT 2
#define CUSTOM_MOUTH 3  


GLfloat custom_head[6][2] = { {0.0, 0.0}, { -5.0, -1.0 },{-4.0,-4.0}, {0.0,-4.5}, {4.0,-4.0},{5.0,-1.0} };
GLfloat custom_lefthat[3][2] = { {-1.0, -1.0 },{ -4.0, -2.0 },{ -3.5, -3.5 } };
GLfloat custom_righthat[3][2] = { {1.0,-1.0}, {4.0,-2.0},{3.5,-3.5} };

//GLfloat custom_mouth[4][2] = { {-2.0,-1.0},{2.0,-1.0},{-2.0,-3.0},{2.0,-3.0} };
GLfloat custom_color[9][3] = {
	{ 183 / 255.0f, 110 / 255.0f, 121 / 255.0f },
	{ 255 / 255.0f, 255 / 255.0f, 255 / 255.0f },
	{ 100 / 255.0f, 100 / 255.0f, 100 / 255.0f },
	{ 102 / 255.0f, 110 / 255.0f, 180 / 255.0f },
	{ 200 / 255.0f, 20 / 255.0f, 20 / 255.0f },
...
GLuint VBO_custom, VAO_custom;
void prepare_custom() {
	GLsizeiptr buffer_size = sizeof(custom_head) + sizeof(custom_lefthat) + sizeof(custom_righthat) /*+ sizeof(custom_mouth)*/;

	
	glGenBuffers(1, &VBO_custom);

	glBindBuffer(GL_ARRAY_BUFFER, VBO_custom);
	glBufferData(GL_ARRAY_BUFFER, buffer_size, NULL, GL_STATIC_DRAW);

	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(custom_head), custom_head);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(custom_head), sizeof(custom_lefthat), custom_lefthat);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(custom_head) + sizeof(custom_lefthat), sizeof(custom_righthat), custom_righthat);
	//glBufferSubData(GL_ARRAY_BUFFER, sizeof(custom_head) + sizeof(custom_lefthat) + sizeof(custom_righthat), sizeof(custom_mouth), custom_mouth);
	glGenVertexArrays(1, &VAO_custom);
	glBindVertexArray(VAO_custom);
...
void draw_custom() {
	...
	glUniform3fv(loc_primitive_color, 1, custom_color[CUSTOM_BODY]);
	glDrawArrays(GL_TRIANGLE_FAN, 0, 6);

	glUniform3fv(loc_primitive_color, 1, custom_color[CUSTOM_RIGHTHAT]);
	glDrawArrays(GL_TRIANGLE_FAN, 6, 5);

	glUniform3fv(loc_primitive_color, 1, custom_color[CUSTOM_LEFTHAT]);
	glDrawArrays(GL_TRIANGLE_FAN, 11, 5);

	
	//glUniform3fv(loc_primitive_color, 1, custom_color[CUSTOM_MOUTH]);
...
void mouse(int button, int state, int x, int y) {
	if ((button == GLUT_LEFT_BUTTON) && (state == GLUT_DOWN))
	{
		flag_start = 1;
		leftbuttonpressed = 1;
		glutTimerFunc(update_rate, time, 0);
	}
...
void time(int)
{
	...
	if (flag_start)
	{
		center_x += sin(rotate_angle) * ROT_SPEED;
		center_y -= cos(rotate_angle) * ROT_SPEED;

		hat_speed -= accelate;
		hat_height += hat_speed;

		cocktail_x += cocktail_x_speed;
		cocktail_y += cocktail_y_speed;
		...
		if (cocktail_y > 0.1f)
		{
			cocktail_y_speed -= accelate;
		}
		else if (cocktail_y < -0.1f)
		{
			cocktail_y_speed += accelate;
		}
		...
		if (cocktail_x > 100)
		{
			cocktail_x_speed = -0.3f;
		}
		else if (cocktail_x < -100)
		{
			cocktail_x_speed = 0.3f;
		}
...
void display(void) {
	...
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3((win_width / 4.0f) + cake_x, (win_height / 4.0f) + cake_y, 0.0f));
	ModelMatrix = glm::rotate(ModelMatrix, -80.0f * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	...
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(win_width / 4.0f + cocktail_x, win_height / 4.0f + cocktail_y, 0.0f));
	...
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-win_width / 4.0f, hat_height, 0.0f));
	ModelMatrix = glm::rotate(ModelMatrix, 30.0f * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	...
	for (i = 0; i < 3; ++i)
	{
		ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(custindex[i], custheight[i], 0.0f));

		ModelMatrix = glm::scale(ModelMatrix, glm::vec3(5.0f, 5.0f, 0.0f));
		ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
		glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
		draw_custom();
	}
	...
	for (i = 0; i < 3; ++i)
	{
		ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(carindex[i], carheight[i], 0.0f));
		ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
		glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
		draw_car();
	}
	...
	for (i = 0; i < 3; ++i)
	{
		ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(car2index[i], car2height[i], 0.0f));
...
void initialize_OpenGL(void) {
	...
	glClearColor(104 / 255.0f, 18 / 255.0f, 49 / 255.0f, 1.0f);
